...
### actor的工作机制1
actor本身有isolated语义(`隔离`), 外界调用actor内部的成员方法时(isolated修饰), 会自动进行隔离, 而明白隔离的过程对理解actor非常重要! 同时这个过程理解后, 也能明白自定义Executor下的MainActor和GlobalActor机制. 
...
### GlobalActor引导2
在上一小节中手动自定义了一个全局的MyGlobalActor协议, 实际上标准库已经实现了一个现成的协议GlobalActor
