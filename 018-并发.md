...
// __code_space_info
  ...
  if (asyncLet) {
    ...
    // asyncLet实际的类型是 AsyncLetImpl*
    // AsyncLet并不是AsyncLetImpl的父类, 但它的空间是抽象出来的, 
...
SWIFT_CC(swiftasync) void main_task_async_main(
    ...
    *(ctx + 0x2c8) = executor.Implementation = MainActor.witnessTable
    ...
    swift_asyncLet_begin(
      ...
      HeapObject*(f1_arg3),
      void*(result_buf) // 28263.83 1074.02554
                         // 8084.28  185.130012
    )
...
在当前小节并不坐探究`swift_asyncLet_finish`的过程. 接下来观察`swift_asyncLet_begin`源码:
...
接收来到`swift_task_create`, 在该函数中将对`closureEntryPoint`进行还原:
...
接下来是很重要的`swift_task_create_commonImpl`, [前面](#link-create_common)已经有部分说明:
...
/*
    ...
    futureResultTypeMetadata: Void.metadata
    function: 第1个异步函数的入口地址, 但真实类型是 void(*)(AsyncContext*, void*)
              只不过当前函数声明的类型是为了统一入口
...
### 什么时候会发生线程切换
通过async main的启动流程, 实际发生线程切换的源码是`swift_task_switch`:
```cpp
#if 0
    该函数表示下一个异步函数在newExecutor上被执行
#endif
SWIFT_CC(swiftasync)
static void swift_task_switchImpl(SWIFT_ASYNC_CONTEXT AsyncContext *resumeContext,
                                  TaskContinuationFunction *resumeFunction,
