...
下面的过程中比较核心的是编译器两次调用`swift_beginAccess`, 由于访问链表存储在TLS中, 所以不同线程所绑定的链表不同, 如果两次`swift_beginAccess`在不同线程调用, 就没有问题:
```swift
