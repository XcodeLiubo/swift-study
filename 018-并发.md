...
而Executor底层被区分成了4种场景:
1. 主线程
2. 没有指定任何隔离区的异步函数
3. actor类型
4. 自定义的GlobalActor

也就是说异步函数被调用的上下文环境处于上述3种场景中的某一个, 所以它们对应的Executor是不同的:

|调用上下文环境|Identity|Implementation|
|:-|:-|:-|
|主线程|`gcd_main_q`|`MainActor.executorWitnessTable`|
|普通异步函数|nullptr,底层会使用优先级不同的并发队列|nullptr|
|actor内部成员|该actor对象的地址|nullptr, 隔离由底层的C++实现|
|遵循GlobalActor的实现|自定义队列|该实现的metadata|
