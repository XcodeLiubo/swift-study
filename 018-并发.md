...
//  同时test-AsyncTask继续执行 n += 30, swift_asyncLet_finish
    ...
    0x100001bf0 <+196>: add    x1, x22, #0x2b8          // x1<ValueBuffer*> = test-ctx + 0x2b8
                                                        //   要传递给 switf_beginAccess的第1个参数, 实际它的类型是Access*
...
->  0x100003a4c <+0>:   orr    x29, x29, #0x1000000000000000
    ...
    0x100003aa0 <+84>:  ldr    x2, [x8, #0x20]          ; x2 = MainActor.witnessTable
...
->  0x1000037a8 <+0>:  mov    x8, x22
    ...
    0x1000037b8 <+16>: ldr    x2, [x8, #0x2d0]          ; MainActor.witnessTable
...
> 只有全局对象的赋值操作都会调用`swift_beginAccess`, 这个操作包括直接赋值和传递inout
...
void swift::swift_task_exitThreadLocalContext(char *state) {
  ...
  if (!oldHead) {
    ...
    //  所以应该将mainThread.TLS.AccessLink复制到main-AsyncTask.Private.ExculsivityAccessSet[0],
