...
第6步中创建`test2-AsyncTask`后直接在subThreadA中执行`flagAsAndEnqueueOnExecutor`之后进入gcd的调度状态, 此刻test函数继续往后执行`swift_asyncLet_finish`, 在该函数中将注册等待机制, 并清空subThreadA的访问链表.  这样第7步启动`test2-AsyncTask`的线程若是subThreadA(取决于gcb的调度), 则`subThreadA.TLS.AccessLink`已经被清空, 所以test2中对n进行赋值时, 没有任何异常. 
...
### actor的工作机制1
actor本身有isolated语义(`隔离`), 外界调用actor内部的成员方法时(isolated修饰), 会自动进行隔离, 而明白隔离的过程对理解actor非常重要! 同时这个过程理解后, 也理能明白自定义Executor下的MainActor和GlobalActor机制. 


```swift
actor A {
    var n = 0
    // actor内部的成员方法默认是isolated属性, 会被编译器认为是一个异步函数
    // PS: 不能直接以 isolated func add() 来定义函数, 默认就是隔离机制,
    //     可以使用 nonisolated 关键字来修饰add函数, 这样add函数就不是一个异步函数
    func add() {
        n += 1
    }
}
...
指令执行完毕上述函数, 会发生调度(`__code_scheduled`):
...
#endif
```
测试中相当于有10000个job同时访问同一个actor对象a, 但最后的结果没有问题, 但还是很耗时
...
这里面最关心的其实是`unownedExecutor`的计算属性, 如果不实现则发起`swift_task_switch`时传递的SerialExecutor是`{actor对象的地址, nullptr}`, 但是如果自己手动实现该方法, 则调度流程将发生变化
...
这个测试主要关注UnownedSerialExecutor的过程以及`swift_task_switch`的过程, 先来看UnownedSerialExecutor:
...
actor A {
    ...
    func add() {}
    nonisolated var unownedExecutor: UnownedSerialExecutor{
        .init(ordinary: e) // __code_exception_2
    }
}
...
### MainActor
MainActor由标准库实现, 但底层要配合C++的源码, 有3部分源码:
1. 接口
2. 源码实现
3. 依赖的底层C++

> 目前只关注接口, 至于实现将在后续具体的案例中探究
...
@globalActor final public actor MainActor : GlobalActor {
    ...
    #if false
    // 由编译器调用, 用户不能调用, 切换任务到 Executor(.Identity = gcd_main_q)
    @inlinable
    public nonisolated func enqueue(_ job: UnownedJob) {
        _enqueueOnMain(job) // 映射到swift_task_enqueueMainExecutor
    }
    #endif
...
这表示对该元素的访问是线程安全的, 当然这个过程由编译器配合Swift的并发来完成, 稍后也会详细探究原理.  Swift提供了@MainActor关键字来显示指定所要修饰的元素.
...
### MainActor的Executor
从目前了解的源码来看, Swift的并发架构大致是:
1. 调度单位是异步函数
2. 异步函数的顺序调用构成异步任务
3. 异步任务运行在某个Executor中
4. Executor在Mac平台上内部关联的是gcd的队列
    > 如果是MainActor则标准库实现了一个`DispatchQueueShim<SerailExecutor>`, 如果自定义的GlobalActor则还会有Implementation指向这个实现(<font color = red>后续详细学习</font>)
...
|自定义GlobalActor|可以随意指定|可以找到`enqueue`地址的witnessTable|
...
这个测试是从Swift的层面验证MainActor的Executor的Identity为`gcd_main_q`. 关于Executor的牵扯到C++和Swift互相调用机制(自定义Executor), 在该小节不会展开. 现在关心的是MainActor的Executor的获取过程
...
...
```
[C++的实现](#link-get-main-executor). 而UnownedSerialExecutor实际的类型是其实是底层C++的Executor:
...
需要清楚的了解f函数是怎么从子线程切换到主线程的详细过程, 这里涉及到`swift_task_enqueue`源码特定的Executor的操作. 原来的源码注解中是普通异步函数在并发队列的切换过程, 并对没有MainActor的异步函数作注解. 再将对应的源码贴出来:
...
```
这里有个疑问: 编译器调用enqueue时传递的this是`gcd_main_q`!!! 解释一下: 编译器是强制调用, this是什么类型由编译器决定, 当然后面肯定是需要`gcd_main_q`这个值
...
### 模拟MainActor的机制
目前所知异步任务一定运行在某个Executor上, 具体来说运行在该`Executor.Identity`(gcd队列)上, 而Swift层面的Executor是依赖于底层C++的Executor, 虽然2者的Executor是不同的语言环境, 但实际上从内存布局的角度来看, 2者是一样的. 先来看C++层面的2种Executor
...
||Swift|C++|
|-|-|-|
|MainActor|DispatchQueueShim|SerialExecutor|
|前8字节|`Metadata*`|`HeapObject*`|
|后8字节|引用计数|`uintptr_t`|

从内存大小上来看2者一样, 但从类型的字段来看2者在逻辑上是不统一的. 在运行的过程中DispatchQueueShim的内存数据是:
1. gcd_main_q
2. `MainActor.executorWitnessTable`

前8字节的gcd_main_q还可以看作一个OC对象, 但后8字节在Swift层面看来并不是一个正确的引用计数, 但整个处理流程中后8字节其实和引用计数也没多大关系, 基于此可以自定义MyMainActor来模拟MainActor的行为. 为了理解这个过程, 笔者将从3个层次来逐步深入, 这样才能明白GlobalActor自定义的一般格式为什么是第3种:
1. 套壳
2. 强制修改前8字节的队列(利用指针)
3. 正确的自定义格式
...
前面2种自定义其实是投机取巧, 正确自定义GlobalActor有固定的格式, 这个格式是有原理的: 前8字节不一定gcd的队列, 后8字节才是核心!!! [回顾前面的MainActor的调度过程](#link-main-actor-impl), 这里面隐藏了一个容易忽略的地方:
1. 当前状态this指向的内存`{.Identity = gcd_main_q, .Implementation = MainActor.executorWitnessTable}`
2. 但是在跳转向C++的`_enqueueOnDispatchQueue`实际将this强制转换成了`_queue<HeapObject*>`

也就是说回调到`DispatchQueueShim.enqueue`时可以传递其他的队列, 不一定是this内存中当前的队列!!! 但还存在另一个问题: 后8字节到底是干什么的?? 事实上后8字节就是`DispatchQueueShim.enqueue`的入口地址!! Swift中没有提供构建一个Executor的方法, 但提供了`SerialExecutor`和`TaskExecutor`的协议, 并且该协议有默认的实现, 基于这些自定义的MainActor如下:
...
@globalActor actor MyMainActor: GlobalActor {
    ...
    final class MyMainExecutor: SerialExecutor {
        ...
        #if false
        // 该成员方法有默认的实现, 默认实现的内部其实相当于什么也没做(所以不用考虑当前self的内存是什么东西)!!
        // PS: 该方法的主要作用是在回调到 enqueue时, 向其传递的不同的self(下一小节有应用), 其实说白了就是将self
        //     强制复制为C++底层的SerialExecutor
        func asUnownedSerialExecutor() -> UnownedSerialExecutor {
...
### GlobalActor的自定义
上一小节是利用GlobalActor模拟了MainActor, 内部的executor对象其实是全局的, 因为`__code_callback_enqueue`中用了统一的串行队列! 在开发中应该是每1个actor实例封装隔离固定的数据, 将前一小节的案例修改为:
...
final class MyExecutor: SerialExecutor{
    ...
    func enqueue(_ job: UnownedJob) {
        debugPrint(q)
        q.async(execute: DispatchWorkItem{
            job.runSynchronously(on: self.asUnownedSerialExecutor())
        })
    }
...
@globalActor actor MyActor: GlobalActor {
    // 当使用 @MyActor修饰 函数, 计算属性, 下标, 方法时编译器
    // 会调用shared对象获取Executor
    static let shared = MyActor(q: .init(label: "global_q"))

    private let e: MyExecutor

    // 要封装的数据
    private var data: Int

    // 使用当前Actor实例时, 使用 self.e.q 来隔离数据
    init(q: DispatchQueue) {
        debugPrint(q)
        self.e = .init(q: q)
        self.data = 0
    ...
    #if false
    不同于前一小节(编译器默认实现的), 在当前案例中自己实现了该方法, 
    其实内部的操作很简单的:
        1. 复制self.e的地址放到x0
        2. 找到MyExecutor.witnessTable放到x1
    返回给外界的就是 {.Identity = e对象的地址, .Implementation = MyExecutor.witnessTable}
    #endif
    nonisolated var unownedExecutor: UnownedSerialExecutor {
        return self.e.asUnownedSerialExecutor()
    }
...
var n = MyActor(q: .init(label: "tmp_q"))

// 测试30个并发(必定多线程), 累加300_0000
// 由于是actor的对象, 所以共用的是同一个串行队列(可能在不同的线程, 但 sync的过程一定是线程安全的)
await withTaskGroup(of: Void.self) {
    for _ in 1...30 {
        $0.addTask {
...
0x00000001700b7000
0x00000001701cf000
0x000000016fe87000      // 串行队列派发的线程, 可以发现有些线程是复用的(由gcd的串行队列决定)
3000000                 // 最终的结果是300_0000, 但其实效率很底(原因之前也解释过)
#endif
```
...
@globalActor actor MyActor: GlobalActor {
    ...
    // Keynote-3
    nonisolated var unownedExecutor: UnownedSerialExecutor {
        self.e.asUnownedSerialExecutor()
    }

    // Keynote-4
    // PS: 该方法不会以回调的方式出现在用户层面, 也就是说通过@MyActor修饰的函数
    //     并不会因为回调到这里获取Executor, 而还是读取shared对象, 通过shared.unownedExecutor获取Executor
    static var sharedUnownedExecutor: UnownedSerialExecutor {
        shared.unownedExecutor
    }
}
```
`Keynote-1`: 有2个点需要说明: 可以使用class来实现; 可以省略:GlobalActor

```swift
// 默认遵循了Actor协议
actor MyActorImpl<Data>{
    let e: MyExecutor
    init(e: MyExecutor) {
        self.e = e
    }
    nonisolated func get_e() -> MyExecutor{
        e
    }

    // 如果不实现也有默认的实现, 其实内部:
    //  1. 复制self的地址到x0
    //  2. 复制MyActorImpl.witnessTable到x1
    // 后面进入到 enqueue时, 取出self.e.q入队
    // PS: 如果不实现(使用默认), 则过程在逻辑上也是一样的
    nonisolated var unownedExecutor: UnownedSerialExecutor {
        UnownedSerialExecutor(ordinary: self.e)
    }
}

// 1. @globalActor已经向编译器说明了 MyActor 是遵循 GlobalActor协议的, 所以可以省略
// 2. class必须是不可被继承的(暂且理解为语法规定)
// 3. 必须为 Sendable, Sendable是告诉编译器该类型发送时一定是深复制的(像struct, enum等值类型默认在传递参数时都是bit复制的)
//    但由于class在传递参数时只是指针复制, 所以class传递到某个函数内部不一定是线程安全的, 所以这里使用 @unchecked Sendable
//    是让编译器在传递MyActor对象时不要做Sendable检查, 否则编译不通过
// PS: @globalActor修饰MyActor并不代表MyActor内部的所有方法都附加了 @MyActor
@globalActor final class MyActor: @unchecked Sendable {

    // shared指向了一个Actor(protocol)
    //  > shared不能再使用MyActor初始化, 因为MyActor并不是Actor
    //  > 所有的actor类型默认都遵循Actor协议(Swift的源码中有注解)
    static let shared = MyActorImpl<Void>(e: MyExecutor(q: .init(label: "global_q")))

    // 每个MyActor对应一个Executor
    private let e: MyExecutor
    ...
    // 自己附加@MyActor, 但其实会使用到 shared.e.q 这个队列, 
    // 并不会使用 self.e.q
    @MyActor  func add() {
        data += 1
    }
    ...
    #if false
    // 本来这个属性是遵循Actor的类型所要自定义实现的(当然也有默认实现),
    // 但当前MyActor是class类型, 所以该方法没有任何作用
    nonisolated var unownedExecutor: UnownedSerialExecutor {
        unownedExecutor(executor: e)
    }
    #endif

    #if false
    // 当前类型被@globalActor修饰, 也继续了该实现.
    // 但前面也提到过: 该方法不以回调的方式返回给Swift要用的Executor!!
    // PS: 再次说明: @MyActor修饰的函数在需要获取Executor时, 编译器
    //     是通过share.unowendExecutor获取Executor的, 并不是这个方法
    static var sharedUnownedExecutor: UnownedSerialExecutor {
        MyActor.shared.unownedExecutor
    }
    #endif
} 

var n = MyMainActor(q: .init(label: "tmp_q"))
await withTaskGroup(of: Void.self) {
    for _ in 1...30 {
        $0.addTask {
            for _ in 1...1000 {
                // add方法被@MyActor修饰, 所以编译器会通过 MyActor.shared.unownedExecutor
                // 获取executor, 所以这30_0000次的循环实际上串行队列 global_q上
                // PS: 本例和前一小节的案例是不同的, MyActor是class类型, 前一小节的是actor类型,
                //     所以前一小节的add方法有actor机制(会取 n.unownedExecutor来获取executor),
                //     而n是一个对象, 它有自己的tmp_q
                async let _ =  n.add()
            }
        }
...
如果不实现`Keynote-3`则shared就是一个actor类型, 底层在切换时会使用DefaultActor, 不会进入到enqueue的方法, 所以`global_q`是无效的, 但DefaultActor内部还是有隔离机制. `Keynote-4`已经提到多次了, 它并不是Swift内部向外界获取Executor的方法
