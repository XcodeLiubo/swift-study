...
```swift
// 在async main的环境下获取的隔离区: MainActor.shared(这个就不验证了)
let actor = #isolation
...
@_inheritActorContext @_implicitSelfCapture operation: sending @escaping @isolated(any) () async -> Success
    ...
    // 因为operation被@_inheritActorContext修饰
    // 所以可以让编译器获取传递operation闭包时所处的隔离区, 在当前的测试流程中该值是 MainActor的隔离区
...
void Task.init(
    ...
    op.Executor = executor  // gcd_main_q, MainActor.executorWitnessTable
                            // PS: 也可能为空, 在当前的流程中确实在MainActor的隔离区
...
后续会调用到`swift_task_create_commonImpl`, 这里需要强调一点: 由于向函数传递的op记录了当前上下文所在的隔离区, 所以这样会导致创建异步任务的函数内部运行在该Executor中, 所以整个Task如果运行后,会直接在主线程中
...
其实第3步开始就和前面MainActor一样的过程了, 由于当前测试程序十分简单, 没有async main, 所以第6步异步注册的函数不会调用到, 因为第6步后程序在当前线程中又继续往后执行, 最后程序直接结束
