...
同理Swift的结构化并发中也有全局对象的保护: actor机制. actor是一种Swift的数据类型: 不可被继承的引用类型. 主要实现数据隔离. Swift的并发架构中有2种actor:
1. MainActor
    - Swift6.0后所有的全局对象默认是`MainActor`
2. 用户声明的actor类型, 此刻actor隔离由编译器实现
    - 这种也称为DefaultActor: `SerialExecutor {.Identity = actor对象的地址, .Implementation = nullptr}`
...
@globalActor actor MyMainActor: GlobalActor {
    ...
    final class MyMainExecutor: SerialExecutor {
        ...
        // 其实该方法的主要作用是在回调到 enqueue时, 向其传递的不同的self
...
前面笔者已经不止一次强调过, 前8字节根据场景内容是不一样的, 在该案例中前8字节其实是`__code_actor_executor.asUnownedSerialExecutor`的返回值, 先来说结论:
