...
### GlobalActor引导2
在上一小节中手动自定义了一个全局的MyGlobalActor协议, 实际上标准库已经实现了一个现在的协议GlobalActor
...
final class MyGlobalExecutor: SerialExecutor, SysGlobalExecutor{
    // GlobalActor必须要实现的存储属性, 类型必须是Actor,
    // 这里选取了A
...
现在可以这种实现逻辑有很多问题:
1. 使用class遵循GlobalActor协议时,需要特定在定义一个actor类型以供shared使用
...
Swift并未提供构造它们的初始化器, 但提供了另一个类型`UnownedSerailExecutor`和`UnownedTaskExecutor`:
...
通过初始化器将SerialExecutor强制转换成C++底层的`class SerialExecutorRef`, 这需要保证Swift下的SerialExecutor对象的内存布局必须为class类型, 至此对Executor作出总结:
1. 主线程
2. 普通异步函数
    - 没有任何隔离区的异步函数
3. actor类型
...
### MainActor
MainActor是标准库实现的特殊GlobalActor, 有3部分源码:
1. 接口
2. 源码实现
3. 依赖的底层C++
...
从内存大小上来看2者一样, 这就注定了可以将DispatchQueueShim对象当前作C++的SerialExecutor来使用. 对于前8字节, 当内容是`gcd_main_q`时, 也没有任何问题, 因为`gcd_main_q`本身就是OC对象, 而DispatchQueueShim本身也是class类型, 所以`gcd_main_q`指向DispatchQueueShim的空间没有问题.
